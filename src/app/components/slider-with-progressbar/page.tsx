...
const SliderWithProgressbar = ({ items, title }: SliderProps) => {
  ...
  useEffect(() => {
    const handleResize = () => {
      ...
      if (width < 640) {
        ...
        setItemsPerPage(5); // Extra Large screens (desktop)
      }
    };
  ...
  return (
    <div className="w-full py-8 min-h-[400px] mx-auto max-w-[82%]">
      <h2 className="text-center pb-7 pt-6 text-2xl font-extrabold">{title}</h2>
      <div className="px-4 relative">
        ...
        <div
          className={`relative overflow-hidden min-h-[384px]
  ...
  `}
        ...
        <SliderProgressbar currentPage={currentPage} totalPages={totalPages} />
      </div>
    </div>
  );
};
