...
const SaleProducts = () => {
    return (
        <div className="container mx-auto pt-4">
               <h2 className="text-center text-2xl font-[900]">Grand Opening Sale deals on select products</h2>
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-6 pt-5">
        {items.map((item, index) => (
          <div key={index} className="flex flex-col items-center text-center">
            <Image src={item.image} alt={item.text} width={120} height={120}
             className="w-28 h-28 object-cover rounded-md mb-2" />
            <p className="text-sm pt-4 text-[#0045bd]">{item.text}</p>
          </div>
        ))}
      </div>
      </div>
